...
#define toHex( x )        ( char ) (((x) > 9 ) ? (x) + 'A' - 10 : (x) + '0' )
	...
	void SetColor( bool bold, int foreground, int background )
	{
		...
		if( bold )
		{
			fputs( "\033[1m", console );
		}
		if( foreground != COLOR_DEFAULT )
		{
			fprintf( console, "\033[%dm", 30 + foreground );
		}
		if( background != COLOR_DEFAULT )
		{
			fprintf( console, "\033[%dm", 40 + background );
		}
		fflush( console );
	}
...
static WORD                       Attributes[ MAX_SCREEN_WIDTH ];
	...
	void SetColor( bool bold, int foreground, int background )
	{
		...
		if( foreground & COLOR_RED )
		{
			attr |= FOREGROUND_RED;
		}
		if( foreground & COLOR_GREEN )
		{
			attr |= FOREGROUND_GREEN;
		}
		if( foreground & COLOR_BLUE )
		{
